...
import { useEffect, useState } from "react"; // Added ReactNode for better typing
      ...
      <div className="prose prose-sm max-w-none text-black">
        <ReactMarkdown
          ...
          components={{
            ...
            h3: (props) => (
              <h3
                className="text-sm font-medium text-black mt-3 mb-1"
                {...props}
              />
            ),
            p: (props) => (
              <p
                className="text-sm leading-relaxed text-black mb-2"
                {...props}
              />
            ),
            li: (props) => (
              <li
                className="text-sm leading-relaxed text-black mb-1"
                {...props}
              />
            ),
            strong: (props) => (
        ...
        {isStreaming && (
          <div className="flex items-center mt-2">
            <span className="inline-block w-2 h-2 bg-gray-200 rounded-full animate-bounce mr-1" />
